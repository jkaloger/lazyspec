...
- Given an edge row, when the designer drills in, then `from`, `to`, `via`, `traversal`, `required`, and `require_to_status` are all editable.
...
- Given an edit that would produce a load error (unknown type, contradictory traversal, `required` on wildcard `from`), when the designer commits it, then the TUI rejects it with the same message the loader would give — not a different wording.
