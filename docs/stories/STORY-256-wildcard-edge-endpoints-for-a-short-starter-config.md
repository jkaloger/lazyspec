...
- Given two matching rows of equal specificity that each state `required` and state different severities, when the config loads, then load fails naming both rows by `name`. An explicit error is preferred over an unpredictable precedence rule (ADR-031). A row that omits `required` states no requiredness and so ties with nothing.
