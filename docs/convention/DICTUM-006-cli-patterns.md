...
- Every command gets its own module under `cli/`. Examples: `cli/show.rs`, `cli/validate.rs`
...
- Every command that produces output must support `--json` for agent consumption. Agents are first-class consumers
