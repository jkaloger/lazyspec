...
- **What the wizard must not prompt for.** `traversal` and `required` are the two fields a new user has no basis to answer, and both have a safe absence: no `required` means "legal, not demanded" (RFC-067 §Design) and no `traversal` means the edge does not walk. Prompt for them with unset as the default, except that `chain` is the default offered while no row the config declares states a traversal at all -- the first row included -- because a from-scratch project whose every edge has no traversal has a `context` of one document, which is the `blank`-path regression ITERATION-396 §Context recorded and this loop is the place it gets fixed.
...
**Amended 2026-09-02 (ITERATION-397):** the "what the wizard must not prompt for" bullet said `chain` goes in front of the user "only once at least one edge exists", which reads as withholding it from the first row -- the one case where nothing walks and the regression bites hardest. The implemented rule keys off traversal rather than existence: `chain` is the default while no declared row states a `traversal`, and the first row is offered it. The bullet now states that.
