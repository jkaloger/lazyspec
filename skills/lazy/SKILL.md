...
**Two commands assemble it.** `validate --json` says *that* an edge is unsatisfied: `jq '[.errors[], .warnings[]] | map(select(.rule == "unsatisfied-edge"))'` picks those findings out, each carrying `path`, `edge_name`, `from_type`, `to` and `via` as fields. Select on `rule` and read fields; never match on `message`, which is the human render. **Read both arrays.** The row's `required` value decides which one a finding lands in, so that placement *is* the severity you report -- `errors[]` alone silently drops every unsatisfied edge whose row is not required. `config --json` says *which* types satisfy an edge in either direction: `edges[].to` when the crossing goes up (the document you hold needs a parent), and the `from` sides of the rows admitting this type when it goes down (the HARD-GATE's reverse lookup, which already collects them as a set). Only the upward case has a finding, so the downward set comes from `config --json` alone. Both spell a set as `[[edges]]` does: a bare name, a list, or `"*"`.
...
**Multi-hop:** when the type at the far side has no document to link to either, report the whole chain the human must author, nearest hop first -- each hop is a separate human-initiated crossing. **Enumerate at the nearest hop only.** The hop being crossed now gets one line per type; each hop beyond it gets a single line naming its edge and its set, and no lines of its own. Render that set yourself from `edges[].to` in `config --json` -- `to one of: spike, story, bug` for a named list, `to a document of any type` for a `"*"` row. Do not copy a finding's `message`: a later hop has no finding to copy, because the document that would carry one does not exist yet. The choice at a later hop is not live until the earlier one is made, and three types at this hop against two at the next is six chains nobody asked to read.
...
- **Scope to the doc just touched.** `validate` is a whole-repo check and will report pre-existing findings across unrelated documents. Findings are objects, so filter on the field carrying the document: `path` on most rules, `source` on `broken-link`, `paths` on `duplicate-id` and `singleton-violation`. Do not grep `message`. Fix only the broken or dangling relation **this mutation introduced** before continuing. Do not block on pre-existing repo-wide findings.
- **One warning names no document, so that filter drops it:** `rule: "gh-auth"`, raised when the project declares a `github-issues` type and the `gh` CLI is missing or unauthenticated. It carries `rule` and `message` only. It is never something a mutation introduced, so it does not block the loop — but report it once rather than letting the scoping filter swallow it.
