...
fn show_parent_json_includes_children() {
    ...
    let output = lazyspec::cli::show::run_json(&store,"RFC-003", false, 25).unwrap();
...
fn show_child_json_includes_parent() {
    ...
    let output = lazyspec::cli::show::run_json(&store,"RFC-003/appendix", false, 25).unwrap();
    let json: serde_json::Value = serde_json::from_str(&output).unwrap();
...
fn show_parent_json_no_children_field_when_none() {
    ...
    let output = lazyspec::cli::show::run_json(&store,"RFC-010", false, 25).unwrap();
    let json: serde_json::Value = serde_json::from_str(&output).unwrap();
