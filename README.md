...
`add-type` rejects a duplicate name; `set-lifecycle` replaces the whole lifecycle (it is a set, not a merge) and rejects an unknown type; `add-edge` rejects a `name` already in the table, since a row is addressed by its name. The mutators require an already-valid config; run `lazyspec fix --config` first to migrate a legacy one.

`add-edge --json` prints one object — `{ "action": "edge-added", "name": …, "edge": … }` — whose `edge` is the row spelled exactly as `config --json` spells it, so a caller reads what landed without re-reading the config. `--json` is accepted before the subcommand as well as after it.
