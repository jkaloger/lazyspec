...
/// Per-workspace daemon. Holds its socket path, host identity, and a lease
/// releaser used to clean up on graceful shutdown.
pub struct Daemon {
    pub root: PathBuf,
...
impl Daemon {
    ...
    pub fn new(root: &Path, config: &Config) -> Result<Self> {
        ...
        let type_names: Vec<String> = config
            ...
            .map(|t| t.name.clone())
            .collect();
        let engine = LeaseEngine::new(GitCli, coordination);
    ...
    pub fn run(&self, shutdown_rx: Receiver<()>) -> Result<()> {
        ...
        // Order: stop accept thread -> release leases -> unlink socket.
...
mod tests {
    ...
    fn daemon_run_binds_socket_then_drains_on_shutdown() {
        ...
        let daemon = make_daemon(&td, Arc::clone(&releaser));
    ...
    fn daemon_replaces_stale_socket_on_startup() {
        ...
        let releaser = Arc::new(RecordingReleaser::new());
        let daemon = Daemon::with_lease_releaser(
            td.path().to_path_buf(),
            sock_path.clone(),
    ...
    fn daemon_refuses_to_start_when_socket_already_live() {
        ...
        let daemon_a = make_daemon(&td, Arc::clone(&releaser_a));
        ...
        let daemon_b = Daemon::with_lease_releaser(
            td.path().to_path_buf(),
            sock_path.clone(),
